...
def apply_shap_validation(impact: ImpactAnalysis) -> ImpactAnalysis:
    if shap is None:
        return impact
    ...
    try:
        ...
        for c in impact.contributors:
            relevant = [v for k, v in shap_strengths.items() if k.startswith(c.metric)]
            if not relevant:
                continue
            shap_dir = np.sign(np.mean(relevant))
            if (shap_dir > 0 and c.direction == "negative") or (shap_dir < 0 and c.direction == "positive"):
                mismatch += 1
        if mismatch > 0:
        ...
        impact.metadata["shap"] = {"direction_mismatches": mismatch, "shap_variance": float(shap_var)}
